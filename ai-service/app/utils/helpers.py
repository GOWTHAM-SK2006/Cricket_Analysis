...
def format_player_context(context: Optional[Dict[str, Any]]) -> str:
    ...
    if all_players and isinstance(all_players, list):
        ...
        for p in all_players:
            ...
            p_ppi = p.get("ppi", 0.0)
            p_mpi = p.get("mpi", 0.0)
            summary += f"• Player: {p_name} | Role: {p_role} | CPI: {p_cpi:.1f} (PPI: {p_ppi:.1f}, MPI: {p_mpi:.1f})\n"
        summary += "-----------------------------------------------\n\n"
    ...
    if player_name:
        ...
        summary += (
            f"--- TARGET PLAYER PROFILE & PERFORMANCE CONTEXT ---\n"
    ...
    return summary if summary.strip() else "No player details available."
