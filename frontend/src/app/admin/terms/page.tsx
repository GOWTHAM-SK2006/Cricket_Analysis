...
export default function AdminTermsPage() {
  ...
  const fetchTerms = async () => {
    ...
    try {
      ...
      showToast("Could not load Terms & Conditions configuration", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedPayload = {
        ...
        changeLogsJson: JSON.stringify([
          { 
            ...
            action: "Updated platform Terms & Conditions legal document", 
            user: "cpi@admin.com" 
          }
        ])
      };
      ...
      showToast("Terms & Conditions saved & published successfully!", "success");
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Manage legal agreements, terms of service, platform policies, and disclaimers. Edits reflect live on the Coach Terms page (`/terms`).
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-300 text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 transition-all cursor-pointer"
          >
            <Eye className="w-4 h-4 text-slate-500" />
            <span>{showPreview ? "Hide Preview" : "Live Coach Preview"}</span>
          </button>

          <button
            onClick={() => {
              setTermsConfig(DEFAULT_TERMS);
              showToast("Reset to default terms configuration", "info");
            }}
          ...
          <div className="mt-8 border-t-2 border-slate-300 pt-6 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-orange-600 uppercase tracking-wider flex items-center gap-2">
                ...
                Live Coach Preview (/terms)
              </span>
              <span className="text-[10px] text-slate-400 font-bold">Mobile/Desktop Display</span>
            </div>
