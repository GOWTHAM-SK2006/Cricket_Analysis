  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-zinc-400 text-sm">Loading module details...</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-8 px-4 pb-6">
      ...
      {view === "history" && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            <div>
              <h1 className="text-3xl lg:text-[36px] font-extrabold tracking-tight bg-gradient-to-r from-white via-zinc-200 to-zinc-400 bg-clip-text text-transparent">Matches (MPI)</h1>
              <p className="text-zinc-400 text-sm lg:text-[16px] mt-1.5 font-medium">Record match details and calculate player performance index metrics.</p>
            </div>
            <button
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 sm:p-6 backdrop-blur-md space-y-4">
            <h2 className="text-xs font-bold text-white uppercase tracking-wider text-zinc-400">Match History</h2>
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-505 mt-1">
                            {new Date(session.date).toLocaleDateString(undefined, {
                              month: "short",
                              day: "numeric",
                              year: "numeric"
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs text-zinc-400 border-t border-white/5 pt-2">
                        ...
                        <div>
                          <span className="text-[9px] text-zinc-550 block uppercase font-bold">Opponent</span>
      ...
      {view === "create_match" && (
        <div className="space-y-6">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white tracking-tight">Create Match</h1>
              <p className="text-zinc-400 text-sm mt-1">Configure match details and pick participating players.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 backdrop-blur-md h-fit space-y-4 shadow-md">
              <div className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Match Configuration</div>
              
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Match Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Select Team</label>
                  <select
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Match Date</label>
                  <input
                    type="date"
                    value={matchDate}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Opponent Name</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-zinc-400 uppercase tracking-wider block">Venue (Optional)</label>
                  <input
                    type="text"
            ...
            <div className="lg:col-span-2 bg-white/5 border border-white/10 rounded-xl p-5 backdrop-blur-md space-y-4 shadow-md">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <div>
                  <h3 className="text-lg font-bold text-white tracking-tight">Match Squad Selection</h3>
                  <p className="text-zinc-400 text-xs mt-1">Select the players who participated in the match.</p>
                </div>
              ...
              {!selectedTeam ? (
                <div className="py-24 text-center space-y-2">
                  <Users className="w-8 h-8 text-zinc-650 mx-auto" />
                  ...
                  {players.map((player) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3.5 border rounded-xl flex items-center justify-between cursor-pointer transition-all hover:bg-white/[0.04] ${
                          isParticipating 
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-zinc-550 mt-0.5">{player.role}</div>
                        </div>
      ...
      {view === "live_assessment" && selectedTeam && (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <h2 className="text-base font-bold text-white uppercase tracking-wider text-zinc-400 px-1">Active Match Squad</h2>
            <p className="text-zinc-500 text-[10px] uppercase tracking-wider font-semibold px-1 -mt-1.5">Select a player to score their MPI metrics.</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between border-t border-white/5 pt-3">
                      <span className="text-[10px] text-zinc-550 uppercase tracking-wider font-bold">MPI Score</span>
                      {isCompleted ? (
                        <span className="text-emerald-400 font-extrabold text-base">{liveMpi}</span>
                      ) : (
                        <span className="text-zinc-555 text-xs italic">Unassessed</span>
                      )}
                    </div>
                  </div>
                );
      ...
      {view === "match_details" && selectedMatchDetails && (
        <div className="space-y-6">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white tracking-tight">Match Assessment Details</h1>
              <p className="text-zinc-400 text-sm mt-1">Review player scores and performance metrics recorded for this match.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {/* Match info */}
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4 shadow-md">
              <span className="text-[11px] text-zinc-550 font-bold uppercase tracking-wider block">Match Details</span>
              <div className="space-y-3 text-xs">
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Match Name</span>
                  <span className="font-bold text-white text-sm">{selectedMatchDetails.session.name}</span>
                </div>
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Opponent</span>
                  <span className="font-bold text-white">{selectedMatchDetails.session.opponent}</span>
                </div>
                {selectedMatchDetails.session.venue && (
                  <div>
                    ...
                    <span className="font-bold text-white">{selectedMatchDetails.session.venue}</span>
                  </div>
                )}
                <div>
            ...
            {/* Team details */}
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4 shadow-md">
              <span className="text-[11px] text-zinc-550 font-bold uppercase tracking-wider block">Team Info</span>
              <div className="space-y-3 text-xs">
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Team</span>
                  <span className="font-bold text-white text-sm">{selectedMatchDetails.session.team.name}</span>
                </div>
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Level</span>
                  <span className="font-bold text-zinc-350">{selectedMatchDetails.session.team.level}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4 shadow-md">
              <span className="text-[11px] text-zinc-550 font-bold uppercase tracking-wider block">Performance Overview</span>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Assessed</span>
                  <span className="text-xl font-black text-white">{selectedMatchDetails.session.playersAssessed} Players</span>
                </div>
                <div>
                  <span className="text-[10px] text-zinc-500 block uppercase">Average MPI</span>
                  <span className="text-xl font-black text-orange-500">{(selectedMatchDetails.session.averageMpi || 0.0).toFixed(1)}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              {selectedMatchDetails.assessments.map((ass) => (
                <div key={ass.id} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    ...
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-[9px] text-zinc-550 block uppercase font-semibold">DEC</span>
                      <span className="font-bold text-zinc-300">{ass.decisionMaking}</span>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-[9px] text-zinc-550 block uppercase font-semibold">AWA</span>
                      <span className="font-bold text-zinc-300">{ass.gameAwareness}</span>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-[9px] text-zinc-550 block uppercase font-semibold">PRE</span>
                      <span className="font-bold text-zinc-300">{ass.pressureHandling}</span>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-[9px] text-zinc-550 block uppercase font-semibold">CON</span>
                      <span className="font-bold text-zinc-300">{ass.teamContribution}</span>
                    </div>
                    <div className="bg-white/5 p-1.5 rounded border border-white/5">
                      <span className="text-[9px] text-zinc-550 block uppercase font-semibold">IMP</span>
      ...
      {isAssessModalOpen && assessingPlayer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-xl w-full max-w-sm p-5 space-y-4 shadow-2xl relative my-4">
            <button
              onClick={() => {
                setIsAssessModalOpen(false);
                setAssessingPlayer(null);
              }}
              className="absolute top-4 right-4 p-1.5 text-zinc-550 hover:text-white rounded-lg hover:bg-white/5 transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
              ...
              {METRICS.map((metric) => (
                <div key={metric} className="space-y-1.5 bg-white/5 border border-white/5 p-2.5 rounded-lg">
                  <div className="flex justify-between items-center text-[10px]">
                    ...
                    <span className="text-orange-405 font-black text-[10px] text-orange-400 font-extrabold bg-orange-500/10 px-1.5 py-0.5 rounded border border-orange-500/20">
            ...
            {/* Notes */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Match Notes / Comments</label>
              <textarea
                rows={2}
                value={tempNotes}
            ...
            <div className="flex items-center justify-end gap-2.5 border-t border-white/5 pt-3">
              ...
              <button
                ...
                className="h-9 px-4 rounded-lg text-xs font-bold transition-all bg-orange-600 hover:bg-orange-500 text-white flex items-center justify-center gap-1 cursor-pointer"
              >
